# Submitted by Group 096
# Harsh Bhatia (2019AD04086), Alkesh Chaturvedi (2019AD04082), Raza Abbas (2019AD04095)
# Bloxorz game ACI Assignment 1
# Date - 28 Nov 2020 Version 1.1
...
#
# Enter playing board here from the input Map.txt file
# Topmost left tile is at x-coordinate = 1, y-coordinate = 1
#
fileHandler = open('Map.txt',"r")
givenMap = []
  
for nextLine in fileHandler:
    givenMap.append([int(k) for k in nextLine.split()])

givenTerrain = getTerrain(givenMap)

#
# Accept Agent or Block start position as [x-coordinate, y-coordinate] as user input:
...
#
print("This program accepts Agent's initial position (x and y co-ordinates from user)")
print("Please enter below the Agent's location on a valid tile. Pls refer above playing court")

xCo = input("Enter agent's x-coordinate: ")
yCo = input("Enter agent's y-coordinate: ")
#
#
...
print("\nStarting position of agent on playing court is at ( x-coordinate =",xCo, "and y-coordinate =", yCo,")")
print("**************************************************")

if xCo.isnumeric() and yCo.isnumeric():
    ...
    if checkFloor(agent):
        ...
        print("Agent starting position is found to be illegal. Agent is not on a valid tile on playing court")
    ...
    print("Please enter x and y co-ordinates for Agent as Numeric values only...")
    print("Also x and y values should be a valid tile position only for the given playing board")
#
